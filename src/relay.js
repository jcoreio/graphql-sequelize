...
export function idFetcher(sequelize, nodeTypeMapper) {
  return async (globalId, context) => {
    ...
    return model ? model.findByPk(id) : nodeType ? nodeType.type : null
  }
}

export function typeResolver(nodeTypeMapper) {
  return (obj) => {
    const model = getModelOfInstance(obj)
    var type =
      obj.__graphqlType__ ||
      (model
...
function reverseOrder(order) {
  return order.map(([orderAttr, orderDirection]) => [
    orderAttr,
    reverseDirection(orderDirection),
  ])
}
...
function toOffsetCursor(node, index) {
  ...
  const id =
    typeof primaryKeyAttribute === 'string'
...
function tupleComparison(model, attributes, inequality, values) {
  ...
  const queryGenerator = model.queryGenerator || model.QueryGenerator
  const attributesStr = attributes
    .map((attribute) => queryGenerator.quoteIdentifier(attribute))
    .join(', ')
  const escapeOptions = { context: 'SELECT' }
  const valuesStr = attributes
    .map((attribute, i) =>
...
export function createConnectionResolver({
  ...
  if (!resolveNodes)
    resolveNodes = baseResolver(targetMaybeThunk, {
  ...
  const resolver = async (source, args, context, info) => {
    ...
    if (last)
      hasPreviousPage = (await nodesPromise).length > (startOnly ? 1 : last)
...
export function createConnection({
  ...
  const { resolveEdge, resolveConnection } = createConnectionResolver({
    orderBy: orderByEnum,
    target: targetMaybeThunk,
    before,
    after,
