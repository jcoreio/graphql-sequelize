...
describe('relay', function () {
  ...
  before(async function () {
    ...
    const [project, _userA, _userB] = await Promise.all([
      Project.create({
        id: projectId++,
    ...
    await _userA.createTaskItem(generateTask(taskId++))
    await _userA.createTaskItem(generateTask(taskId++))
    await _userA.createTaskItem(generateTask(taskId++))
    await _userB.createTaskItem(generateTask(taskId++))
    await _userB.createTaskItem(generateTask(taskId++))
    const [userA, userB] = await Promise.all([
      User.findOne({ where: { id: _userA.id }, include: [User.Tasks] }),
      User.findOne({ where: { id: _userB.id }, include: [User.Tasks] }),
    ])
            ...
            edges: [
              {
                node: {
                  name:
                    user[User.Tasks.as][user[User.Tasks.as].length - 1].name,
                },
              },
            ],
          },
